...
def new_mentor(model, form_class, template_name, redirect_url, redirect_id=None, post_save_callback=None):
...
def create_record(model, form_class, template_name, redirect_url, redirect_id=None, post_save_callback=None):
    def decorator(view_func):
        ...
        def _wrapped_view(request, *args, **kwargs):
            if request.method == "POST":
                # Initialize form with POST data, FILES, and current user
